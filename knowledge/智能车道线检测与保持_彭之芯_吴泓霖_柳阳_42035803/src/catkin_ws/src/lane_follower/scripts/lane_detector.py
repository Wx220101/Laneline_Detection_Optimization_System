...
class LaneDetector:
    def __init__(self):
        ...
        self.output_video = rospy.get_param('~output_video', '')
        self.output_fps = rospy.get_param('~output_fps', 10)
        self._writer = None
    ...
    def _ensure_writer(self, frame):
        if not self.output_video or self._writer is not None:
            return
        h, w = frame.shape[:2]
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        self._writer = cv2.VideoWriter(self.output_video, fourcc, float(self.output_fps), (w, h))
        if not self._writer.isOpened():
            rospy.logerr(f"Failed to open VideoWriter: {self.output_video}")
            self._writer = None
    ...
    def image_callback(self, msg):
        ...
        self._ensure_writer(cv_image)
        if self._writer is not None:
            self._writer.write(cv_image)
        if self.show:
